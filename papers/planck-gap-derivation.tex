...
floor \(\tau\Delta E\ge(1-2\epsilon)^2\hbar^2/(4A)\) without any
...
to meet. Replace the decision criterion by a hypothesis test: two
prepared ensembles, a fixed error probability \(\epsilon\), and a
criterion that the data distinguish \(\mathrm I\) from \(\mathrm F\) at
level \(\epsilon\). Replace \(\delta\) and \(\Delta\) by the error and
