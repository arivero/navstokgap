...
\textbf{Correction, 2026-09-22.} Section 4 of the first version applied
Theorem B to every Gaussian probe. Theorem B assumes that each mark's
error and recoil are independent, and a probe whose position and
momentum are correlated delivers correlated ones. The corrected
statement is Theorem C: with correlation coefficient \(\rho\) between a
mark's error and its recoil, the cost that enters the floor is
...
along a rotated quadrature has \(\rho\to1\) and decides the comparison
at any force (Proposition D), and Yuen's contractive states are this
resource. The same mechanism corrects the aperture corollary of the
\href{planck-gap-probabilistic.md}{probabilistic note}: in both places a
floor of order \(\hbar\) holds once the shape of the uncertainty ellipse
is bounded, and its area alone leaves the floor free.

\textbf{Sharpening, 2026-09-22.} The constant of Theorem B improves from
\(\frac29\) to \(\frac1{24}\) in \(d^2\,m\kappa/F^2\tau^3\). The
invariance under \(v_0\) makes the function \(T\) of the proof vanish at
both ends, so the Poincaré constant \(\tau^3/12\) replaces the earlier
Cauchy--Schwarz bound \(\frac49\tau^3\). Dense protocols attain the new
constant, so \(\tau\Delta E\ge48z^2\kappa\) is sharp.
