...
A correlation \(\rho\) between a mark's error and its recoil multiplies
the floor by \(\sqrt{(1-\rho)/(1+\rho)}\), and a three-mark protocol
attains that dependence, so the floor is a statement about marks whose
...
Three marks at \(0,\tau/2,\tau\) with the test \(R_1-2R_2+R_3\) attain
this up to the constant, since the outer marks' recoils are invisible to
the test and the middle mark's noise \(-2\xi_2+(\tau/2m)\iota_2\) is one
