...
fundamental alone, gives the same form with \(6340\) in place of
\(176\). The entropy constant \(20e\) for plaquette animals is the only
place with room, and a careful count would lower \(176\) to about
...
\textbf{All representations.} The level of \(r\) is the least \(n(r)\)
with \(r\subset(f\oplus\bar f)^{\otimes n}\), and
...
a bound \(36\) times worse than the leading activity. Everything below
is stated for a per-plaquette activity \(\rho\); the two readings differ
only in what \(\rho\) is.
...
all representations, \(\rho=36/g^2\) & \(g^2\ge6340\) &
\(\Delta_W\ge(\hbar c/a)\,4\log(g^2/6340)\) \\
\end{longtable}

The first line uses the leading term of \(c_f/c_0\) and drops the higher
representations, which contribute at relative order \(\beta_W\); making
it rigorous costs a factor \((1+O(1/g^2))\) in \(\rho\), not the factor
\(36\). The second line is rigorous as written.
...
and the intermediate region between \(g^2\sim10^{-2}\) and
