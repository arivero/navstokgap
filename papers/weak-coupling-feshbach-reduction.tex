...
\(\eta\le Cg^{4/3}\hbar c/L\); (H3) the effective constant-mode
Hamiltonian \(PHP\), which is C133's operator plus the nonzero-mode
zero-point energy as a function of the constant modes, has gap
\(\ge\delta_1^{(3)}g^{2/3}(1-Cg^{2/3})\hbar c/L\). The scaling analysis
of the cubic and quartic couplings shows why each inequality is expected
with the stated powers, and identifies the smallness parameter as
\(g^{2/3}\), Lüscher's expansion parameter. The lemma is proved; the
three inequalities are the open content of the lower side of the
small-volume corner. Units: \(\hbar=c=1\) in this note, energies in
units of \(\hbar c/L\) and \(g\) dimensionless, as in
...
(The constant-mode part of \(\int|[\tilde A,\tilde A]|^2\) is not
separated further; it belongs to \(W_4\) and is quartic in the nonzero
modes.) The Gauss constraint is imposed on states; the constant gauge
...
lower side is (H2), the Schur error of \(W_2\) and \(W_3\) against the
