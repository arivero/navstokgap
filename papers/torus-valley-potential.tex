...
by \(O(\hbar c/L)\) globally, periodic with centre-degenerate minima.
The same Poisson computation for non-commuting backgrounds requires the
Landau-level spectrum of the covariant Laplacian with constant field
strength and the Nielsen--Olesen mode, which is the next input; on the
abelian valley the reduction's (H3) reduces to a relatively bounded
perturbation of \(h_3\) by \(-|C_M|g^{4/3}|\xi|^2/(3\pi^2L)\) inside the
region \(La\ll1\) and by a bounded potential outside it, which the
confining bound of C133 controls.
\end{document}
