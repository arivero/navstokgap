...
\(\tau_*=(9z_{1-\epsilon}^2m\hbar/F^2)^{1/3}\), has the same form,
...
\emph{Conjecture, to be checked against the text.} Newton's fits supply
a Newton-age form of that premise. In his emission theory the corpuscle
...
\(\Lambda p/2\pi\), and two paths with equal endpoints then arrive with
fits phases differing by their enclosed area over \(\Lambda p/2\pi\).
The relevant passages of Book II are not yet in the
\href{../docs/classics/Newton_Opticks_1730_fits_and_queries.md}{Opticks
companion}, and a Maupertuis phase \(\int p\,dq\) gives twice the
quantum per-step value \(F^2\tau_j^3/24m\), since the quantum phase is
the full action difference.
...
\(\Phi_N+4\theta_N=\tau\Delta E/3\hbar\). Next: the Newton-age form of
the central extension through the fits (§5), which needs the Book II
passages on plates in different media; the general force law, where
Lemma 2 holds unchanged and Theorem 1's segment areas become the areas
between the phase-plane path and its chords; and a paper section
