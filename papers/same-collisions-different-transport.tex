...
The contribution is an elementary comparison at matched collision frequency:
the same elastic mechanics and one-particle kinetic scales support either
ordinary diffusion or exact periodic cancellation. Sections 3 and 4 derive
the two trajectories and their variances; Table~\ref{tab:comparison} collects
what is held fixed and what changes. The required tools are equal-mass elastic
collisions, exponential waiting times and integration of a velocity covariance.
...
origin without changing the macroscopic density. The velocity process $V(t)$
is stationary: its joint law is unchanged by shifting all observation times.
The displacement increments $X(t+s)-X(t)$ consequently have laws independent
of $t$. The position process pinned at $X(0)=0$ is not stationary.
...
preparation. Equal values of the mean complete collision interval and velocity variance do
