...
measure with a Euclidean measure, carried out in
\href{ground-state-measure-transfer.md}{the transfer note}:
\(|\Omega|^2d\mu\) is the time-slice marginal of the four-dimensional
Wilson measure, so every local Euclidean estimate transfers, and the
local large-field bound becomes available in its standard
scale-invariant form \(e^{-c\eta^2/g^2}\).
\end{document}
