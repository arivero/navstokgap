...
large \(L\). The first theorem-sized target this leaves is T2 in
Hamiltonian form: a volume-uniform gap for \(g\ge g_0(N)\), whose radius
\(g_0\) is the number the entire problem asks to push to zero. Sources
are cited inline with reading labels; nothing here is promoted.
...
\(N\) & Euclidean form established (Osterwalder--Seiler); Hamiltonian
form is the next target \\
...
coupling for the Kogut--Susskind Hamiltonian, by a cluster expansion or
a relatively bounded perturbation argument that handles the unbounded
