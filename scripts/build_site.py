...
Research repository <a href="{repo}">arivero/navstokgap</a>. Pages generated by
<code>scripts/build_site.py</code>; every note is also the Markdown source in
<code>notes/</code> and a typeset PDF in <code>out/papers/</code>.
Notes are working research, not peer-reviewed publications; each states its own
status and the premises it uses.
...
def main():
    ...
    (DOCS / "ledger.html").write_text(page(
        ...
        "<h1>Claim ledger</h1>" + pandoc(rewrite_links(led, slugs))),
        encoding="utf-8")

    src = (ROOT / "docs/classics/README.md").read_text(encoding="utf-8")
    src = src.split("\n", 1)[1] if src.startswith("# ") else src
    src = LINK.sub(lambda m: (m.group(0) if m.group(1).startswith("http")
                              else f"]({BLOB}/docs/classics/{m.group(1)})"
                              if not m.group(1).startswith("../")
                              else f"]({BLOB}/{m.group(1)[3:]})"), src)
    (DOCS / "sources.html").write_text(page(
        ...
        "Byte identity is registered in <code>docs/SHA256SUMS</code>.</p>"
    ...
    print(f"site: {len(notes)} notes, {len(pdfs)} papers, "
