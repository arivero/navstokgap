...
frequencies $|k\pm a|$ over $k\in(2\pi/L)\mathbb Z^3$; the divergent part
...
with $C_M=\lim_{b\to0}[\sum'_me^{im\cdot b}|m|^{-2}-2\pi^2/|b|]$, so
$U(a)=2|a|+\tfrac{C_M}{3\pi^2}L|a|^2+\cdots$. The linear term $2|a|$ is
exactly the $k=0$ contribution, four oscillators of frequency $|a|$
with zero-point energy $\tfrac12|a|$ each, which are the transverse
...
along the valley is linear only for $La\ll1$, saturates at order $1/L$,
...
gap is $g^{2/3}/L\ll1/L$, the ground state is localized at $La\sim g^{2/3}$
...
charged parts of $\tilde A_2,\tilde A_3$, four real oscillators of
frequency $|a|$, while the charged parts of $\tilde A_1$ are gauge
...
[G07](low-dimensional-mass-gap.md) Proposition 7 this frequency is $|a|$.
The zero-point energy $4\cdot\tfrac12|a|=2|a|$ is the confining term of
...
=\sum_{k\in(2\pi/L)\mathbb Z^3}\big(|k+a|+|k-a|-2|k|\big),$$
where the $k=0$ term is $2|a|$ and the neutral modes cancel. Each term is
...
U(a)=2|a|+\frac{C_M}{3\pi^2}\,L|a|^2+O(L^2|a|^3).$$
The linear term $2|a|$ is the $k=0$ contribution of Section 1, so the
nonzero modes contribute
$$U_\perp(a)=U(a)-2|a|=\frac{C_M}{3\pi^2}L|a|^2+O(L^2|a|^3),$$
...
$U(a)\le2\max\Phi/(\pi^2L)=O(1/L)$ for all $a$, while the $k=0$ term $2|a|$
alone grows without bound. The linear confinement of the constant modes
...
connected by tunnelling through the valley barrier of height $O(1/L)$,
...
has width $La\sim g^{2/3}$ and energy $g^{2/3}/L$, far below the valley
saturation scale $1/L$ and the tunnelling barrier, so the C133 picture
...
energy reaches $1/L$, $g(L)^{2/3}\sim1$, is where the holonomy
...
spectrum of the covariant Laplacian with constant field strength and
the Nielsen--Olesen mode, which is the next input; on the abelian valley
the reduction's (H3) reduces to a relatively bounded perturbation of
$h_3$ by $-|C_M|g^{4/3}|\xi|^2/(3\pi^2L)$ inside the region $La\ll1$ and
