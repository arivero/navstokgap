...
$2mL+\tau A/L$ over $L$, which yields the largest lower bound, attained
only at the balanced aperture $L=\tau P/2m$. For an aperture of aspect
$r=L/\sqrt{\tau A/2m}$ the floor is $(1-2\epsilon)^2\hbar^2/(A(r+r^{-1})^2)$,
and a squeezed Gaussian of area $\hbar/2$ and large $r$ drives it to
zero. Theorems 1 and 2, stated in $L$ and $P$ separately, are unaffected;
Corollary 3, the lead and Sections 5--6 now carry the balanced form.
...
the area $A$ controls it only once the shape is fixed.
...
- **The resource is named and the bound is tight in it.** Ozawa's
  ...
  exchange at each aperture $(L,P)$, and Proposition 4 shows the price is
  right, so the standard quantum limit appears here as the balanced
