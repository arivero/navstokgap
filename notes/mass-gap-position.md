...
| T4 | construction |
| local large-field control | needs the Euclidean weight or a transfer-matrix identification |
...
([blocking note](blocking-step-obstruction.md) §5). The decimation step
requires a local large-field estimate, and the notes show that the
...
$\Omega^2$ with a Euclidean measure on a time slice through the transfer
matrix, and work there.
